...
#
# Latest version:
# https://raw.githubusercontent.com/Karlson2k/k2k-OSX-Tools/master/Create_macos_install_iso/create_osx_install_iso_v2.sh
#
# Version 2.0.1
...
Valid options are:
      -a, --app[lication] <macOS Install app | download>
      ...
      -o, -overwrite
                   Overwrite output .iso, if present.
      -m, --method <D>
                   ...
                   Method 2 create more BIOS/EFI-friendly images.
      ...
      -r, -reuse   Reuse already downloaded files in download directory,
                   if size exactly matched size on server, do not resume
                   partially downloaded files
      -rr, -resume Reuse already downloaded files in download directory,
                   if size exactly matched size on server and resume
                   partially downloaded files. Dangerous, files on server
                   may differ from previously partially downloaded files!
      -r-          Do not reuse or resume files in download directory
      -p           Push downloaded files to \"lite\" upgrade application
      -p-          Do not push downloaded files to \"lite\" upgrade
...
unset cmd_par_app cmd_par_iso test_name ver_opt cr_method ovwrt_iso cust_script || exit_with_error "Can't unset variable"
...
while [[ -n "$1" ]]; do
	case "$1" in
		...
		-i | --iso ) cmd_par_iso="$2"
			...
			shift 2 ;;
		-o | --overwrite ) ovwrt_iso='yes'; shift ;;
...
[[ "${cr_method-notset}" == "notset" ]] || [[ "$cr_method" =~ ^"method"[1-4]$ ]] || exit_with_cmd_err "Unknown creation method specified: ${cr_method#method}"
[[ $cust_script == "yes" ]] && [[ "$cr_method" == "method4" ]] && exit_with_cmd_err "Custom helper script is not compatible with method 4"
...
download_install_files() {
	...
	# First iteration: check for existing files and decide whether to reuse thme
	num_files=${#dwnl_list[@]}
	for ((i=0; i<=$num_files; i++)); do
		file_name=${dwnl_list[$i]##*/}
		case "${file_name}" in
			InstallESDDmg.pkg | BaseSystem.dmg | AppleDiagnostics.dmg ) : ;; # download files
			BaseSystem.chunklist | AppleDiagnostics.chunklist | InstallInfo.plist ) : ;; # download files
			*) 	unset dwnl_list[$i] && unset dwnl_sizes[$i] || exit_with_error "Can't unset variable" # skip file
				continue ;;
		esac
		if [[ -f "$work_dir/$file_name" ]]; then
			if [[ $(stat -f %z "$work_dir/$file_name") -eq "${dwnl_sizes[$i]}" ]]; then
				if [[ -z $reuse_files ]]; then
					...
					case "$answer" in
						'y' | 'Y') reuse_files='yes' ;;
						'n' | 'N') reuse_files='no' && resume_download='no' ;;
					esac
				fi
				...
				echo "If you choose 'yes' then all existing files with matched names will be overwritten."
				valid_answers=(y Y n N q)
				read -n 1 -p "[y/n, q for quit]: " answer
				echo ''
				until is_answer_valid $answer ${valid_answers[@]} 'q'; do
				...
				reuse_files='no' && resume_download='no'
				rm "$work_dir/$file_name" || exit_with_error "Can't delete file $work_dir/$file_name"
			fi
	...
	if [[ "$reuse_files" = "no" ]]; then
		rm -f "$work_dir/InstallESDDmg.pkg" "$work_dir/BaseSystem.dmg" "$work_dir/AppleDiagnostics.dmg" || exit_with_error "Can't delete file"
		rm -f "$work_dir/BaseSystem.chunklist" "$work_dir/AppleDiagnostics.chunklist" "$work_dir/nstallInfo.plist" || exit_with_error "Can't delete file"
	fi
	if [[ "$resume_download" = "no" ]]; then
		rm -f "$work_dir/InstallESDDmg.pkg.incomplete" "$work_dir/BaseSystem.dmg.incomplete" \
			"$work_dir/AppleDiagnostics.dmg.incomplete" || exit_with_error "Can't delete file"
	...
	for ((i=0; i<=$num_files; i++)); do
		...
		if [[ -f "$work_dir/$file_name" ]]; then
			...
			echo_warning "Skipping download of existing file $file_name."
		fi
		if [[ -f "$work_dir/$file_name.incomplete" ]]; then
			echo_warning "Download of $file_name will be resumed."
		fi
	done
	dwnl_list=( "${dwnl_list[@]}" ) # rebuild indices 
	dwnl_sizes=( "${dwnl_sizes[@]}" ) # rebuild indices 
	...
	for ((i=0; i<${#dwnl_list[@]}; i++)); do
		...
		if [[ $(stat -f %z "$work_dir/$save_name.incomplete") -ne "${dwnl_sizes[$i]}" ]]; then
		...
		mv "$work_dir/$save_name.incomplete" "$work_dir/$save_name" || exit_with_error
	done
...
unset test_name answer SharedSupport_dir || exit_with_error
if [[ ! -f "$OSX_inst_app/Contents/SharedSupport/InstallESD.dmg" ]] || \
	[[ ! -f "$OSX_inst_app/Contents/SharedSupport/BaseSystem.dmg" ]] || \
	[[ ! -f "$OSX_inst_app/Contents/SharedSupport/BaseSystem.chunklist" ]] || \
	[[ ! -f "$OSX_inst_app/Contents/SharedSupport/InstallInfo.plist" ]]	; then
	SharedSupport_dir="$OSX_inst_app/Contents/SharedSupport"
	if [[ $(sed -n -e '\|<key>CFBundleShortVersionString</key>| { N; s|^.*<string>\([0-9]\{1,\}\)\..*</string>.*$|\1|p; q; }' \
		...
		download_install_files || exit_with_error
		unset downloaded_ver install_app_ver || exit_with_error
		install_app_ver=$(sed -n -e '\|<key>DTSDKName</key>| { N; s|^.*<string>macosx\(.*\)internal</string>.*$|\1|p; q; }' \
			"$OSX_inst_app/Contents/Info.plist") || unset install_app_ver
		downloaded_ver=$(sed -n -e '\|<key>System Image Info</key>|,\|</dict>| { \|<key>version</key>|{ N; s|^.*<string>\([0-9]\{1,\}\.[0-9]\{1,\}\)\..*</string>.*$|\1|p; q; };}' \
			"$work_dir/InstallInfo.plist") || unset downloaded_ver
		...
		$sudo_prf bash -c "sed -e '\|<key>version</key>| {n; s|<string>10.\([0-9]*\(\.[1-9]\{1,\}\)\{0,1\}\)\..*</string>|<string>10.\1</string>|;}' \
			-e 's|InstallESDDmg.pkg|InstallESD.dmg|' -e 's|InstallESDDmg|InstallESD|' -e '\|<key>chunklist.*</key>| {N; /InstallESD/d; }' \
				'$work_dir/InstallInfo.plist' > '$SharedSupport_dir/InstallInfo.plist'" || exit_with_error "Can't transform InstallInfo.plist"
		rm "$work_dir/InstallInfo.plist" || echo_warning "Can't delete $work_dir/InstallInfo.plist"
		$sudo_prf mv "$work_dir/InstallESDDmg.pkg" "$SharedSupport_dir/InstallESD.dmg" && \
			$sudo_prf mv "$work_dir/BaseSystem.dmg" "$work_dir/AppleDiagnostics.dmg" \
				"$work_dir/BaseSystem.chunklist" "$work_dir/AppleDiagnostics.chunklist" "$SharedSupport_dir/" || exit_with_error "Can't move files to $SharedSupport_dir"
		stage_end_ok "Moving succeed"
	fi
fi
...
if [[ \$_HW_MANUFACTURER = "VMware, Inc." ]]; then
	if [[ -e /dev/disk0 ]]; then
		_HDD_NAME=\$(diskutil info -plist /dev/disk0 | sed -n -e '\|<key>MediaName</key>|{n;s|.*<string>\(.*\)</string>.*$|\1|p;}')
		[[ -z \$_HDD_NAME ]] && _HDD_NAME='Unknown'
		_HDD_PART_TYPE=\$(diskutil info -plist /dev/disk0 | sed -n -e '\|<key>Content</key>|{n;s|.*<string>\(.\{1,\}\)</string>.*$|partitioned with \1|p;s|.*<string></string>|not partitioned|p;q;}')
		[[ -z \$_HDD_PART_TYPE ]] && _HDD_PART_TYPE='unknown paritioned'
		echo "First drive \"\$_HDD_NAME\" is \$_HDD_PART_TYPE" | /usr/bin/logger -t "" -p install.notice
		if [[ \$_HDD_PART_TYPE = "not partitioned" ]]; then
			if [[ \$_HDD_NAME =~ ^'VMware Virtual' ]]; then
				echo "Automatically partitioning empty first VMware virtual drive with GPT and HFS+ partition" | /usr/bin/logger -t "" -p install.notice
				diskutil eraseDisk JHFS+ 'VMware Drive' GPT /dev/disk0 | /usr/bin/logger -t "" -p install.notice
			else
				echo "First hard disk drive \"\$_HDD_NAME\" will not be automatically partitioned as it does not look like virtual drive" | /usr/bin/logger -t "" -p install.notice
			fi
		else
			echo "First hard disk drive \"\$_HDD_NAME\" is already partitioned" | /usr/bin/logger -t "" -p install.notice
		fi
	else
		echo 'No hard disk drives is detected' | /usr/bin/logger -t "" -p install.notice
	fi
else
...
if [[ -e "$OSX_inst_result_image_ro" ]]; then
	if [[ -z $ovwrt_iso ]]; then
		 exit_with_error "\"$OSX_inst_result_image_ro\" already exist"
	 else
		 rm -f "$OSX_inst_result_image_ro" || exit_with_error "Can't delete \"$OSX_inst_result_image_ro\""
	 fi
fi
makehybrid_errout="$tmp_dir/hdiutil_makehybrid_erroutput" || exit_with_error
{ { hdiutil makehybrid -o "$OSX_inst_result_image_ro" "$OSX_inst_img_rw" -hfs -udf -default-volume-name "$OSX_inst_prt_name" \
	  -hfs-blessed-directory /System/Library/CoreServices 2>&1 1>&3 && \
...
if ! [[ -e "$OSX_inst_result_flag" ]]; then
	if fgrep -Fiqs -e 'Operation not permitted' "$makehybrid_errout" && [[ "$have_su_rights" != "yes" ]]; then
		...
		if [[ "$allow_sudo" == "yes" ]]; then
			...
			echo_warning "Will retry with sudo, you may be asked for password."
			$sudo_prf hdiutil makehybrid -o "$OSX_inst_result_image_ro" "$OSX_inst_img_rw" -hfs -udf -default-volume-name "$OSX_inst_prt_name" \
				-hfs-blessed-directory /System/Library/CoreServices  && touch "$OSX_inst_result_flag"
		else
...
if [[ -e "$OSX_inst_result_flag" ]]; then
	...
	if hdiutil resize -sectors min -shrinkonly -nofinalgap "$OSX_inst_img_rw"; then
		stage_end_ok "succeed"
	else
